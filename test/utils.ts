...
import { Deployer } from '@matterlabs/hardhat-deploy';
// @ts-ignore
 ...
 * Available on In-memory node and Dockerized node.
 */
...
export const expectRevert = async (txFunc, contract, error) => {
    await expect(txFunc()).to.be.revertedWithCustomError(contract, error);
};
